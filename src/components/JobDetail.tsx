  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900">{job.title[currentLanguage]}</h2>
        <button
          onClick={onClose}
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="flex items-center">
            ...
            <span className="ml-2 text-gray-600">{job.department[currentLanguage]}</span>
          </div>
          <div className="flex items-center">
            <span className="font-medium text-gray-700">Posted</span>
            <span className="ml-2 text-gray-600">{new Date(job.postedDate).toLocaleDateString(currentLanguage === 'pt' ? 'pt-AO' : currentLanguage === 'fr' ? 'fr-FR' : 'en-US')}</span>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <ul className="list-disc list-inside text-gray-600 space-y-2">
            {job.requirements[currentLanguage].map((req: string, index: number) => (
              <li key={index}>{req}</li>
            ))}
          </ul>
        </div>
        ...
        <div className="space-y-4">
          ...
          <ul className="list-disc list-inside text-gray-600 space-y-2">
            {job.benefits[currentLanguage].map((benefit: string, index: number) => (
